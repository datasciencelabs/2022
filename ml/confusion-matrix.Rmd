...
- We use the __caret__ package, which has several useful functions for building and assessing machine learning methods and we introduce in more detail in Section \@ref(caret).
...
- We are calling almost half of the females male! How can our overall accuracy be so high then?  This is because the _prevalence_ of males in this dataset is high.
...
$$\frac{1}{\frac{\beta^2}{1+\beta^2}\frac{1}{\mbox{recall}} + Balanced accuracy and $F_1$ score
...
- However, in Chapter \@ref(cross-validation) we describe an approach to estimating the MSE that tries to mimic this theoretical quantity.
