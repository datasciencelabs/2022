...
- We can generate an example with three categories.
...
```{r mnist-27-training-data, eval=FALSE} 
...
- is much worse because the model is more rigid.

- This is what the decision rule looks like:
...
- `r caret::confusionMatrix(predict(train_knn, test_set), test_set$y)$overall["Accuracy"]`.
...
```{r three-classes-lack-of-fit, eval=FALSE} 
